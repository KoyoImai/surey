...
def train(opt, model, model2, criterion, optimizer, scheduler, dataloader, epoch, method_tools):
    ...
    if opt.method == "er":
        ...
        classil_acc, taskil_acc = val_lucir(opt, model, model2, criterion, optimizer, scheduler, train_loader, val_loader, epoch)
        ncm_acc = ncm_lucir(model, ncm_loader, val_loader)

        logger.info(f"task {opt.target_task} Epoch {epoch}: train_loss={loss:.4f}, ClassIL_accuracy={classil_acc:.3f}, TaskIL_accuracy={taskil_acc:.3f}, NCM_accuracy={ncm_acc:.3f}")
